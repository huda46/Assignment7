...
// TODO: get staffId is todo! (no solution yet)
...
firstnameEl.addEventListener("input", function () {
  firstnameEl.setCustomValidity( Staff.checkName( firstnameEl.value).message);
});
lastnameEl.addEventListener("input", function () {
  lastnameEl.setCustomValidity( Staff.checkName( lastnameEl.value).message);
});
typeEl.addEventListener("input", function () {
  typeEl.setCustomValidity( Staff.checkType( typeEl.value).message);
});
...
commitBtn.addEventListener("click", async function () {
  const slots = {
    personId: formUpEl["staffId"].value,
    firstname: formUpEl["firstname"].value,
    lastname: formUpEl["lastname"].value,
    type: formUpEl["type"].value
  };

  formUpEl["firstname"].setCustomValidity( Staff.checkName( slots.firstname).message);
  formUpEl["lastname"].setCustomValidity( Staff.checkName( slots.lastname).message);
  formUpEl["type"].setCustomValidity( Staff.checkType( slots.type).message);
...
// TODO: get staffId! (no solution yet)
